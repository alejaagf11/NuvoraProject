...
@Component({
  selector: 'app-transacciones-mobile',
  standalone: false,
  templateUrl: './transacciones-mobile.component.html',
...
export class TransaccionesMobileComponent implements OnInit {
  ...
  if (this.categoriasFiltradas.length === 0) {
    ...
    if (!this.nuevaTransaccion.montoTransaccion || !this.nuevaTransaccion.descTransaccion || !this.nuevaTransaccion.categoriaId) {
    ...
    if (this.isEditMode && this.transaccionEditandoId) {
      this.transaccionService.update(this.transaccionEditandoId, this.nuevaTransaccion).subscribe({
        next: () => {
          this.mensajeExito = 'Transaccion actualizada correctamente';
          this.errorMensaje = null;
        ...
        error: (err) => {
          ...
          this.errorMensaje = err.error?.message || 'No se pudo actualizar la transaccion';
        }
      });
      return;
    }
    ...
    this.transaccionService.create(this.nuevaTransaccion).subscribe({
      next: () => {
        this.mensajeExito = 'Transaccion registrada correctamente';
        this.errorMensaje = null;
      ...
      error: (err) => {
        ...
        this.errorMensaje = err.error?.message || 'No se pudo guardar la transaccion';
      }
    });
  }
  ...
  eliminarTransaccion(id?: number) {
    if (!id || !confirm('Seguro que quieres eliminar esta transaccion?')) return;

    this.transaccionService.delete(id).subscribe({
      next: () => {
        this.mensajeExito = 'Transaccion eliminada correctamente';
        this.errorMensaje = null;
        this.cargarTransacciones();
      },
      error: (err) => {
        console.error('Error al eliminar transaccion', err);
        this.errorMensaje = 'No se pudo eliminar la transaccion';
      }
    });
  }
